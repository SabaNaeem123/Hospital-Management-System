...
/* Gradient Pulse Effect on Focus */
@keyframes gradientPulse {
    0% { background: linear-gradient(135deg, #FF4500, #FF7043); }
    50% { background: linear-gradient(135deg, #FF7043, #FF4500); }
    100% { background: linear-gradient(135deg, #FF4500, #FF7043); }
}
...
function fetchDoctors() {
    ...
    if (specialization) {
        fetch(`get_doctors.php?specialization=${specialization}`)
...
function fetchTimeSlots() {
    ...
    const timeSlotSelect = document.getElementById('time_range');
    const typeInput = document.getElementById('type');
    timeSlotSelect.innerHTML = "<option value=''>Select Time Slot</option>"; // Reset time options
    typeInput.innerHTML = ""; // Reset type input
    ...
    if (doctorId) {
        fetch(`get_slots.php?doctor_id=${doctorId}`)
            ...
            .then(data => {
                data.forEach(slot => {
                    // Format the display with available date
                    timeSlotSelect.innerHTML += `<option value="${slot.available_date} ${slot.start_time}|${slot.end_time}">${slot.available_date} - ${slot.start_time} - ${slot.end_time}</option>`;
                    typeInput.innerHTML += `<input type="hidden" name="appointment_type" value="${slot.type}">`; // Corrected input tag
                });
            });
    }
}
...
<form action="book_appointment.php" method="POST" id="form">
    <label for="specialization">Select Specialty:</label>
    <select name="specialization" id="specialization" onchange="fetchDoctors()" required>
        <option value="">Select Specialty</option>
        <?php while ($row = $result_specialties->fetch_assoc()): ?>
            <option value="<?= $row['specialization'] ?>"><?= $row['specialization'] ?></option>
        <?php endwhile; ?>
    </select>

    <label for="doctor_id">Select Doctor:</label>
    <select name="doctor_id" id="doctor_id" onchange="fetchTimeSlots()" required>
        <option value="">Select Doctor</option>
    </select>

    <label for="time_range">Select Time Slot:</label>
    <select name="time_range" id="time_range" required>
        <option value="">Select Time Slot</option>
    </select>
<input type="text" id="type" name="appointment_type" hidden>
    ...
    <button type="submit" name="add">Book Appointment</button>
</form>
