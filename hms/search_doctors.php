...
$patient_id = $_SESSION['id']; // Fetch patient ID from session
...
if (isset($_POST['search'])) {
  ...
  $sql = "SELECT d.id, d.name, d.specialization 
          FROM doctors d
          JOIN doctor_daily_main_availability m ON d.id = m.doctor_id 
          WHERE 1=1"; // Start with a true condition

  // Initialize an array for binding parameters
  $params = [];

  // Append specialization condition if provided
  if (!empty($specialization)) {
      $sql .= " AND d.specialization = ?";
      $params[] = $specialization;
  }

  // Append availability condition if provided
  if (!empty($availability_date)) {
      $sql .= " AND m.available_date = ?";
      $params[] = $availability_date;
  }

  // Append status condition (assuming we only want available doctors)
...
// Fetch all specialties for the initial selection
$sql_specialties = "SELECT DISTINCT specialization FROM doctors";
$result = $conn->query($sql_specialties);
?>
...
if (isset($_POST['search'])) {
    ...
    $sql = "SELECT d.id, d.name, d.specialization 
            FROM doctors d
            LEFT JOIN doctor_daily_main_availability m ON d.id = m.doctor_id 
            WHERE 1=1"; // Start with a true condition
    ...
    // Append status condition (assuming we only want available doctors)
    $sql .= " AND (m.status = 'available' OR m.status IS NULL)"; // Include doctors with no availability records
...
// Fetch all specialties for the initial selection
$sql_specialties = "SELECT DISTINCT specialization FROM doctors";
$specialties = $conn->query($sql_specialties);
?>


    <h2>Search Doctors</h2>
    <form method="POST" style="margin-top: 5rem; margin-bottom: 5rem; max-width: 400px; margin-left: auto; margin-right: auto; padding: 2rem; background-color: #FFF3E0; border-radius: 8px; box-shadow: 0 4px 8px rgba(0, 0, 0, 0.1);">
    <label for="specialization" style="font-weight: bold; color: #BF360C; margin-bottom: 0.5rem; display: block;">Specialization:</label>
    <select name="specialization" style="width: 100%; padding: 0.5rem; border-radius: 4px; border: 1px solid #FF7043; margin-bottom: 1rem; font-size: 1rem; background-color: #FFCCBC; color: #BF360C;">
        <option value="">Select Specialization</option>
        <?php while ($row = $specialties->fetch_assoc()): ?>
            <option value="<?= htmlspecialchars($row['specialization']) ?>"><?= htmlspecialchars($row['specialization']) ?></option>
        <?php endwhile; ?>
    </select>

    <label for="availability_date" hidden>Availability Date:</label>
        <input type="date" name="availability_date" hidden>

    <button type="submit" name="search" style="width: 100%; padding: 0.75rem; background-color: #E64A19; color: white; border: none; border-radius: 4px; font-size: 1rem; cursor: pointer; transition: background-color 0.3s;">
        Search
    </button>
</form>
    ...
    <?php if (isset($result) && $result->num_rows > 0): ?>
        <h3 style="text-align: center; color:rgb(234, 78, 55)">Available Doctors</h3>
        <table>
            <thead>
                <tr>
                    <th>Doctor Name</th>
            ...
            <tbody>
                <?php while ($row = $result->fetch_assoc()): ?>
                    <tr>
                        ...
                        <td>
                            <a class="btn edit-btn" href="book_appointment.php?doctor_id=<?php echo $row['id']; ?>">Book Appointment</a>
                        </td>
                    </tr>
                <?php endwhile; ?>
            </tbody>
