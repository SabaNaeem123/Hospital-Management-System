...
if (!isset($_SESSION['admin']) && !isset($_SESSION['doctor'])) {
    header('Location: doctor_login.php');  // Redirect to the relevant login if not authorized
    exit();
}
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if ($stmt->execute()) {
        if ($registered_by == 'admin') {
            ...
            header('Location: doctor_home.php');  // Redirect to admin dashboard
        }
         else {
...
<?php include 'doctor_home.php'; ?>
    ...
    <input type="hidden" name="registered_by" value="doctor" style="display: none;"> <!-- Hidden field -->
